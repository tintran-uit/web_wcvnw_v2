...
class ProductController extends Controller
{
    ...
    public function getProducts()
	{
		...
		{
		 	$products_list[""+$cat->id] = DB::select('SELECT tr.`farmer_id` "farmer_id", f.`name` "farmer_name", p.`id` "id" ,p.`name` "name", p.`slug` "slug", p.`image` "image", p.`thumbnail` "thumbnail", p.`price` "price", p.`unit_quantity` "unit_quantity", IF(tr.`capacity` - tr.`sold` - p.`unit_quantity` <= 0, 0, round(tr.`capacity` - tr.`sold`, 1)) AS "quantity_left", p.`unit` "unit", b.`name` "label"  FROM `products` p, `trading` tr, `farmers` f, `brands` b WHERE tr.`product_id` = p.`id` AND p.`brand_id` = b.`id` AND f.`id` = tr.`farmer_id` AND p.`category` = ?', [$cat->id]);
