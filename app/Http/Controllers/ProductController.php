		...
		if (strcmp($locale, 'en') == 0) {
			...
			foreach($categories as $cat)
			{
			 	$products_list[""+$cat->id] = DB::select('SELECT tr.`farmer_id` "farmer_id", f.`en_name` "farmer_name", 
			 													 p.`id` "id" ,p.`en_name` "name", p.`slug` "slug", p.`price_old` "price_old" ,p.`image` "image", p.`thumbnail` "thumbnail", tr.`price` "price", tr.`unit_quantity` "unit_quantity", IF(tr.`capacity` - tr.`sold` - tr.`unit_quantity` < 0, 0, round(tr.`capacity` - tr.`sold`, 1)) AS "quantity_left", tr.`en_unit` "unit", p.`brand_id` "label"  
