...
class FarmerController extends Controller
{
	...
	{
		$trading = DB::select('SELECT p.`name` "product_name", t.`capacity` "capacity", t.`sold` "sold", t.`unit` FROM `trading` t, `products` p 
			WHERE t.`product_id` = p.`id` AND t.`status` = 1 AND t.`farmer_id` = ?', [$farmer_id]);
