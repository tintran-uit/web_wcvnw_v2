...
public function statsByDate($date)
  ...
  public function layhang($id, $date)
  {
    $products_list = DB::select('SELECT tr.`farmer_id` "farmer_id", f.`name` "farmer_name", p.`id` "id" ,p.`name` "name",
                                        p.`slug` "slug", p.`image` "image", p.`thumbnail` "thumbnail", tr.`price` "price",
  ...
  {
    $products_list = DB::select('SELECT tr.`farmer_id` "farmer_id", f.`name` "farmer_name", p.`id` "id" ,p.`name` "name",
                                        p.`slug` "slug", p.`image` "image", p.`thumbnail` "thumbnail", 
                                        tr.`price` "price", tr.`unit_quantity` "unit_quantity", tr.`sold` "sold", 
